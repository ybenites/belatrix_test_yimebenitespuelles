...
// var url_data = route.getCurrentPage.key

var management = new Management(Vue, {
  debug: debug
})
...
router.beforeEach((to, from, next) => {
  route.path = to.path
  // url_data = route.getCurrentPage.key
  store.commit('identifier', route.getCurrentPage.identifier)
  next()
})
...
new Vue({
  ...
  methods: {
    handleResize(event) {
      ...
      store.commit('fullHeight', fullHeight)
      store.commit('fullWidth', fullWidth)
